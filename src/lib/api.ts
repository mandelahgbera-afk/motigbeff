...
export const api = {
  ...
  copyTrades: {
    ...
    distributeProfit: async (copyTradeId: string, profitAmount: number, traderFeesPct: number) => {
      ...
      const { data: ct } = await supabase.from('copy_trades').select('*').eq('id', copyTradeId).single();
      if (!ct) throw new Error('Copy trade not found');
      
      // Calculate user's share (100% - trader fee)
      const userShare = (profitAmount * (100 - traderFeesPct)) / 100;
      
      // Add profit to user's balance
      const current = await this.getByEmail(ct.user_email);
      const balance = await supabase.from('user_balances').select('*').eq('user_email', ct.user_email).single();
      if (balance.data) {
        await supabase
          .from('user_balances')
          .update({
            balance_usd: (balance.data.balance_usd || 0) + userShare,
            total_profit_loss: (balance.data.total_profit_loss || 0) + userShare,
            updated_at: new Date().toISOString(),
          })
          .eq('user_email', ct.user_email);
      }
